...
function renderMarkdown(raw: string): string {
  ...
  // Replace display math $$...$$ (multiline)
  text = text.replace(/\$\$([\s\S]*?)\$\$/g, (_match, math) => {
    const placeholder = `<div class="katex-display">${renderMath(math.trim(), true)}</div>`;
    return placeholder;
  });
...
export function getResearchPage(
  ...
  const filePath = path.join(CONTENT_ROOT, projectSlug, `${pageSlug}.md`);
...
export function getResearchLogEntry(
  ...
  const filePath = path.join(CONTENT_ROOT, projectSlug, "log", `${entrySlug}.md`);
...
export function getResearchLogEntries(projectSlug: string): ResearchPageData[] {
  const logDir = path.join(CONTENT_ROOT, projectSlug, "log");
